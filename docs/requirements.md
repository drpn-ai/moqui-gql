...
6. **Aggregation / analytics — DEFERRED (Q2 RESOLVED).** BI facts (SUM/COUNT/time-series) are a
   ...
   initial scope.
7. **Computed / service-backed fields.** Genuine aggregations/state machines and ATP — shipped examples: `Order.orderItemCount`, `inventoryLevels` (ATP). (`customerName` is a join+concat, reclassified as the leaf `billToCustomer` edge per the leaf-over-service rule, not service-backed.) Validates decision 12.
