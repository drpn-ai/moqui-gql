...
| [`docs/shopify-alignment.md`](docs/shopify-alignment.md) | Validation of our schema/query language against the GraphQL spec and **Shopify Admin GraphQL** — alignment scorecard, the low-controversy changes to become Shopify-shaped (Relay PageInfo, `sortKey`+`reverse`, `MoneyV2`/`MoneyBag`, scalars, naming), and the two open decisions (filtering style; global IDs). |
