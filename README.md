...
| [`docs/examples.md`](docs/examples.md) | **The capability catalog and test-case basis.** ~22 worked GraphQL examples across every domain (Order, Shipment, Picklist, Returns, Transfers, Inventory/ATP, Catalog+Search, Facility, CycleCount, Routing, analytics, external-id, agent, feed) plus the guardrail/rejected cases. Each example carries its expected response shape, entity/view/service mapping, cost class, and concrete **test assertions**. |
