...
| [`docs/implementation-plan-inventory.md`](docs/implementation-plan-inventory.md) | The TDD plan for #35 — converting the `inventoryLevels` root from a service-backed list into a **view-entity-backed Relay connection** over `ProductFacilityInventoryItemView` (ATP/QOH per configured product+facility, 0-not-null), and retiring the example service + inventory-key governor cap. |
